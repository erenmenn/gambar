...
## 4. Data Preparation
Setiap film dapat memiliki **lebih dari satu genre**, sehingga diperlukan proses **one-hot encoding** untuk mengubah daftar genre menjadi kolom-kolom biner (0/1). 

Setelah genre diproses menjadi format biner (`movies_expanded`), data tersebut perlu **digabungkan dengan dataset ratings** menggunakan `movieId` sebagai kunci penggabungan menggunakan merge.

Tujuannya adalah Memastikan setiap aktivitas rating memiliki informasi lengkap dan Menghubungkan judul film dengan rating pengguna

### Kolom Identifikasi & Rating
Setelah proses penggabungan, dataset final (df) memiliki struktur kolom yang komprehensif dan siap untuk analisis lebih lanjut:
...
### Genre Splitting & Penggabungan Dataset

Seperti yang dijelaskan sebelumnya, sebelum membentuk kolom final kita melakukan **genre splitting** menggunakan metode `str.get_dummies(sep='|')`. Teknik ini mengubah representasi genre dari format multiclass menjadi multi-label biner. Transformasi ini esensial karena setiap film dapat memiliki lebih dari satu genre secara simultan, dan representasi biner memfasilitasi perhitungan kemiripan antar film menggunakan metrik seperti **cosine similarity** atau **Jaccard index**.

Setelah genre diproses, dilakukan **merge tables** antara dataset `ratings` dan `movies` menggunakan `movieId` sebagai kunci penggabungan melalui operasi `pd.merge(ratings, movies, on='movieId')`. Proses merge ini memastikan bahwa setiap baris rating tidak hanya berisi informasi numerik penilaian pengguna, tetapi juga metadata film yang lengkap meliputi judul dan informasi genre dalam format biner.

### Pembersihan Data

Langkah selanjutnya adalah **penghapusan kolom redundant** dengan melakukan `drop(columns=['timestamp', '(no genres listed)'])`. Kolom `timestamp` dihilangkan karena tidak memberikan signal yang signifikan bagi model sistem rekomendasi, terutama untuk pendekatan content-based filtering yang berfokus pada kesamaan atribut konten. Sementara itu, kolom `(no genres listed)` di drop karna juag tidak memberikan kontribusi/kosong.

Untuk mencegah perhitungan similarity yang berulang, dilakukan **drop duplikasi film**.Hal ini penting karena dalam dataset ratings, satu film dapat muncul berkali-kali dengan rating dari pengguna yang berbeda. Namun, untuk matriks kemiripan antar film, setiap film cukup direpresentasikan sekali saja.



### Encoding , Normalisasi, data split

Tahap akhir preprocessing melibatkan **encoding dan scaling** data. `LabelEncoder`

Pada Collaborative Filtering, data yang digunakan hanya mencakup userId, movieId, dan rating. Kedua ID dikonversi menjadi indeks numerik menggunakan Label Encoding, dan rating dinormalisasi ke rentang 0–1 agar sesuai dengan fungsi aktivasi sigmoid. Setelah itu, fitur (user, movie) dan target (rating_norm) dipisahkan,

 lalu dibagi menjadi data train–validation dengan rasio **80:20**. Data yang telah tersiapkan ini kemudian digunakan sebagai input untuk melatih model Coll_
...
### Contoh Hasil dari content filtering
...
Dataset dibagi menjadi **80% training dan 20% validasi**. Model dilatih menggunakan **optimizer Adam** dengan **loss function MSE** dan **metrik evaluasi MAE**. Untuk mencegah overfitting, diterapkan **early stopping** dengan patience 3 epoch yang akan menghentikan training jika validation loss tidak membaik selama 3 epoch berturut-turut.
...
## 6.1 Evaluasi Collaborative Filtering
...
**Precision@K** mengukur seberapa banyak dari K rekomendasi teratas yang benar-benar relevan bagi pengguna. Metrik ini menjawab pertanyaan: "Dari semua rekomendasi yang diberikan, berapa persen yang benar-benar disukai pengguna?" Semakin tinggi precision, semakin akurat model dalam memberikan rekomendasi yang tepat sasaran.
...
**Recall@K** mengukur seberapa banyak item relevan yang berhasil ditemukan oleh model dari seluruh item relevan yang dimiliki pengguna. Metrik ini menjawab: "Dari semua film yang seharusnya direkomendasikan, berapa persen yang berhasil masuk ke daftar rekomendasi?" Recall yang tinggi menunjukkan model mampu menemukan sebagian besar film yang relevan untuk pengguna.
...
**F1@K** adalah harmonic mean dari precision dan recall, digunakan untuk menilai keseimbangan keduanya. Metrik ini penting karena precision dan recall sering kali memiliki trade-off: meningkatkan satu metrik bisa menurunkan yang lain. F1 memberikan gambaran menyeluruh tentang performa model.
...
## 6.2 Evaluasi Content-Based Filtering
...
Pada tahap ini, genre film dijadikan sebagai ground-truth atau acuan kebenaran untuk menentukan apakah rekomendasi yang diberikan relevan atau tidak. Genre dianggap sebagai label karena sistem hanya memiliki fitur genre biner, sehingga tidak ada fitur lain yang dapat dijadikan pembanding.

Setelah genre film asli diperoleh, langkah berikutnya adalah mengambil daftar film yang paling mirip dengan film tersebut berdasarkan cosine similarity. Kode yang digunakan adalah:
```python
sims = cos_sim_df_subset.loc[idx].drop(idx).sort_values(ascending=False).head(topn)
```

Kode ini berfungsi untuk menghasilkan daftar Top-N rekomendasi berdasarkan nilai cosine similarity tertinggi. Film yang sedang dievaluasi sendiri tidak diikutsertakan dalam hasil rekomendasi (di-skip menggunakan `drop(idx)`), karena tidak masuk akal merekomendasikan film yang sama. Hasilnya diurutkan dari similarity tertinggi ke terendah, lalu diambil sejumlah N teratas (dalam kasus ini N=5).

Tahap selanjutnya adalah menentukan apakah sebuah rekomendasi relevan atau tidak. Proses ini dilakukan dengan membandingkan genre film rekomendasi dengan genre film asli:
```python
